...
import Regular3.Representations
...
instance BiFunctor Id where
  bifmap f g (Id r) = Id (g r)

instance BiFunctor Param where
  bifmap f g (Param r) = Param (f r)
...
instance (BiFunctor f, BiFunctor g) => BiFunctor (f :+: g) where
  bifmap f g(L x) = L (bifmap f g x)
  bifmap f g(R y) = R (bifmap f g y)

instance (BiFunctor f, BiFunctor g) => BiFunctor (f :*: g) where
  bifmap f g (x :*: y) = bifmap f g x :*: bifmap f g y

instance BiFunctor f => BiFunctor (Con f) where
  bifmap f g (Con con r) = Con con (bifmap f g r)

--instance (Functor f) => Functor (PSet f) where
--  fmap f (PSet xs) = PSet (S.map (fmap f) xs)

instance BiFunctor f => BiFunctor (f :^: a) where
  bifmap f g (Exp mp) = Exp $ M.map (bifmap f g) mp
...
class Crush f where
  crush :: (a -> b -> b) -> b -> f a r -> b

instance Crush Id where
  crush op e (Id x) = crush op e x

instance Crush Param where
  crush op e (Param x) = (x `op`)

instance Crush (K k) where
  crush _ e _ = e
